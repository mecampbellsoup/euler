...
grid_array.each_with_index do |row, i|
  row.each_with_index do |elem, j|
    ...
    down            = grid_array[i+1][j] if i < row.size-1
    diag_down_left  = grid_array[i+1][j-1] if i < row.size-1 && j > 0
    diag_down_right = grid_array[i+1][j+1] if i < row.size-1 && j < row.size-1

    elem.right           = right
    elem.down            = down
    elem.diag_down_left  = diag_down_left
